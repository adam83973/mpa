...
def assignment_to_lesson
  ...
  #Matches assignment with lesson and adds resource id to lesson.assignemnt if it hasn't already
  #been added
  ...
  #Matches assignment key with lesson and adds resource id to lesson.assignemnt_key if it hasn't already
  #been added
  @assignments.each do |i|
    l = lesson_regex.match(i.filename)
    if l
      @lessons = Lesson.where("week = ?", "#{l[:week]}")
      @lessons.each do |lesson|
        if lesson.standard.course.course_name == l[:course]
